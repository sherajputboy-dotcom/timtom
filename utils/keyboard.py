#!/usr/bin/env python3
"""Colorfully Formatted & Styled Keyboard Builders for Lenskart Bot (Powered by MoneyZone)"""
...
def main_menu_keyboard() -> InlineKeyboardMarkup:
    """High-contrast colorful inline menu keyboard."""
    keyboard = [
...
def force_join_keyboard(channels: list, verified: set = None) -> InlineKeyboardMarkup:
    """Build Force Join channel buttons with color status badges and direct clickable links."""
...
def channel_list_keyboard(channels: list) -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton("◀️ 🔙 Back to Channels", callback_data="admin_channels")])
    return InlineKeyboardMarkup(buttons)
